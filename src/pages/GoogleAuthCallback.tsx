...
export default function GoogleAuthCallback() {
  ...
  useEffect(() => {
    ...
    if (token) {
      const decodedEmail = getEmailFromJwt(token) || email || 'google-user@manto.local'
    ...
    } else if (error === 'token_failed' || error === 'userinfo_failed' || error === 'server_error') {
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-slate-950 text-white">
      <div className="w-full max-w-md rounded-2xl border border-white/10 bg-white/5 p-8 text-center shadow-[0_20px_60px_rgba(0,0,0,0.35)] backdrop-blur-xl">
        ...
        <h1 className="mt-4 text-xl font-semibold tracking-tight">Google Authentication</h1>
